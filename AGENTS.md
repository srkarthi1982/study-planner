...
- 2026-02-27 Middleware Standard V1 normalization: added config-driven middleware (`src/lib/middlewareConfig.ts` + shared `src/middleware.ts`) with identical invariants (static asset bypass contract, production cookie-domain enforcement, normalized auth flow order, safe numeric admin role check, DEV-only bypass guard). Preserved study-planner behavior via `protectMost`, current public auth routes, and `/api/faqs.json` bypass; DEV bypass semantics unchanged (`DEV_BYPASS_AUTH`). Verification: `npm run typecheck` ✅, `npm run build` ✅.
